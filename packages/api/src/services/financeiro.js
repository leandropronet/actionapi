...
async function listar({ tipo, filialId, status, vencimentoDe, vencimentoAte, page = 1, pageSize = 100 }) {
  const tabela = tipo === 'CP' ? 'raw.financeiro_cp' : tipo === 'CR' ? 'raw.financeiro_cr' : null;

  if (!tabela) {
    return listarConsolidado({ filialId, status, vencimentoDe, vencimentoAte, page, pageSize });
  }
  ...
  const [dataRes, countRes] = await Promise.all([
    db.query(
      `SELECT id, filial_id, data_emissao, data_vencimento, _dados, _sync_at
       FROM ${tabela} ${where}
...
async function listarConsolidado({ filialId, status, vencimentoDe, vencimentoAte, page, pageSize }) {
  const buildWhere = (params) => {
    ...
    if (status) { params.push(status); c.push(`_dados->>'STATUS' = $${params.length}`); }
  ...
  const [cpRes, crRes] = await Promise.all([
    db.query(`SELECT 'CP' AS tipo, id, filial_id, data_emissao, data_vencimento, _dados FROM raw.financeiro_cp ${whereCp} ORDER BY data_vencimento LIMIT ${pageSize} OFFSET ${offset}`, paramsCp),
    db.query(`SELECT 'CR' AS tipo, id, filial_id, data_emissao, data_vencimento, _dados FROM raw.financeiro_cr ${whereCr} ORDER BY data_vencimento LIMIT ${pageSize} OFFSET ${offset}`, paramsCr),
  ]);
...
async function fluxoCaixa({ dataInicio, dataFim, filialId }) {
  ...
  const [cpRes, crRes] = await Promise.all([
    db.query(
      `SELECT DATE_TRUNC('day', data_vencimento) AS dia, SUM((_dados->>'VALOR')::NUMERIC) AS valor
